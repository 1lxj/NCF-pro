...
if __name__=='__main__':
    ...
    # model = dense_att3.DenseLayer(hidden_list, 943, 1682, embedding_size, dae_size=50, drop_ratio=0.14)
    ...
    #       top_k=10)
    #
    # f = open('dense_att3,{},{},{},{}.pickle'.format(lr, dense_reg, cross_reg, cross_ratio),'wb')
    # pickle.dump(model,f)
    # f.close()
    ...
    list_ndcg = []
    list_f1 = []
    with open('./model/dense_att3,0.007,0.08,0.05,0.4,0.14,50,6.pickle', 'rb') as f:
        model = pickle.load(f)
        for k in range(5, 31, 5):
            ndcg, f1, a, b = test(model, test_matrix, k)
            list_ndcg.append(ndcg)
            list_f1.append(f1)
    with open('model_100k', 'w') as a:
        a.write(list_ndcg)
        a.write('\n')
        a.write(list_f1)
