...
def train(model, data_train, data_test, epoch, optim, top_k):
    ...
    for i in range(200):
        ...
        for x, y, s in data_train:
            ...
            optim.zero_grad()
            regularization_dense, regularization_dae = regularization(model)
            loss_ncf = Loss_ncf(out, s) + 0.001 * regularization_dense
            loss_dae = Loss_dae(corrupt, true) + 0.05 * regularization_dae
            loss = loss_ncf + 0.4 * loss_dae
            loss.backward()
            optim.epoch()
        if (i + 1) % 5 == 0 :
            print(i)
...
def regularization(model):
    ...
    return math.sqrt(loss_dense), math.sqrt(loss_dae)
...
if __name__=='__main__':
    ...
    model = dae_ncf.dae_ncf(943, 1682, embedding_size)

    optim = torch.optim.Adam(model.parameters(), lr=0.001)
    train(model, train_data, test_matrix, epoch=200, optim=optim, top_k=10)