...
class DenseLayer(nn.Module):
    def __init__(self, nb_hidden_list, user_num, item_num, embedding_size, dae_size, drop_ratio):
        ...
        self.DAE = nn.Sequential(
            nn.Linear(embedding_size *2, dae_size),
            nn.Sigmoid(),
            nn.Linear(dae_size, embedding_size * 2),
            nn.Sigmoid()
        )
    ...
    def forward(self, user_vector, item_vector):
        ...
        input_dense = self.DAE(embedding)
        out_hidden_dict = {0: input_dense}
        ...
        return out_hidden_dict[len(self.nb_hidden_list) - 1], input_dense, embedding
